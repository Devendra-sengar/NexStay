...
// ─── PATCH /api/hostel-admin/my-hostels/:id/settings ───────────────────────
export const updateMyHostelSettings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!hostel) {
      res.status(404).json({ success: false, message: 'Hostel not found or unauthorized' }); 
      return;
    }
