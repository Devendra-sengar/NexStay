...
// ─── GET /api/student/rent/:id/receipt ───────────────────────────────────────
export const getStudentRentReceipt = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const studentRecord = await HostelStudent.findOne({ guestId: req.user?.id }).lean();
    if (!studentRecord) { res.status(404).json({ success: false, message: 'Student not found' }); return; }

    const record = await RentRecord.findOne({ _id: req.params.id, hostelStudentId: studentRecord._id })
