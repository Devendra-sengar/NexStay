...
import { useAdminManagers, useUpdateUserStatus } from '@/hooks/useAdmin';
import { Search, UserCheck, UserX, AlertTriangle, ShieldCheck, UserCog } from 'lucide-react';
...
export default function AdminManagersPage() {
  ...
  const { data: managers, isLoading } = useAdminManagers(search);
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      toast.success(`Manager account has been ${nextStatus.toLowerCase()} successfully.`);
    } catch {
      toast.error('Failed to update manager account status.');
    } finally {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-primary tracking-tight">Property Managers</h2>
          <p className="text-text-muted text-sm">Control platform access privileges for property managers and staff.</p>
        </div>
      ...
      <div className="bg-surface-card border border-surface-border rounded-2xl overflow-hidden shadow-xl">
        {isLoading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-surface-border text-sm text-text-secondary">
                {managers.map((manager: any) => (
                  <tr key={manager._id} className="hover:bg-surface-dark/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-text-primary">{manager.name}</div>
                      <div className="text-xs text-text-faint">ID: {manager._id}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div>{manager.email}</div>
                      <div className="text-xs text-text-faint">{manager.phone}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      {manager.status === 'ACTIVE' ? (
                        <button
                          onClick={() => setConfirmModal({ isOpen: true, userId: manager._id, userName: manager.name, currentStatus: 'ACTIVE' })}
                        ...
                        <button
                          onClick={() => setConfirmModal({ isOpen: true, userId: manager._id, userName: manager.name, currentStatus: 'SUSPENDED' })}
          ...
          <EmptyState
            ...
            description={search ? `No property managers matched "${search}". Try another search term.` : "There are currently no registered Property Managers on the platform."}
          />
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface-card border border-surface-border rounded-2xl max-w-md w-full p-6 space-y-6 shadow-2xl animate-scale-in">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-text-primary">
                  {confirmModal.currentStatus === 'ACTIVE' ? 'Suspend Manager Account' : 'Reactivate Manager Account'}
                </h3>
            ...
            <p className="text-sm text-text-secondary leading-relaxed">
              Are you sure you want to {confirmModal.currentStatus === 'ACTIVE' ? 'suspend' : 'reactivate'} the manager account for{' '}
