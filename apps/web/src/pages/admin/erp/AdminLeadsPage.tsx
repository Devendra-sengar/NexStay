...
import { useGetLeads, useCreateLead, useUpdateLeadStatus, useDeleteLead, useAdminProperties } from '@/lib/adminApi';
import { format } from 'date-fns';
import { Plus, Search, MapPin, X, Loader2, Phone, Mail, UserCircle, Trash2 } from 'lucide-react';
...
export default function AdminLeadsPage() {
  ...
  const { data: leads, isLoading } = useGetLeads({ search, propertyId, status });
  const { data: propertiesResp } = useAdminProperties();
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">Manage walk-ins and property enquiries</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select className="input-field sm:w-48" value={propertyId} onChange={(e) => setPropertyId(e.target.value)}>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-surface/50 text-text-muted border-b border-surface-border">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Property / Source</th>
            ...
            <tbody className="divide-y divide-surface-border">
              {isLoading ? (
                ...
                leads?.map((lead) => (
                  <tr key={lead._id} className="hover:bg-surface/30 transition-colors group">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-1.5 font-medium text-text-primary">
                        ...
                        {lead.propertyId?.name || lead.hostelId?.name || 'Any'}
                      </div>
                    ...
                    <td className="px-4 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleDelete(lead._id, lead.name)} className="p-1.5 text-text-muted hover:text-danger hover:bg-danger/10 rounded-lg">
...
function LeadModal({ open, onClose, properties }: { open: boolean; onClose: () => void; properties: any[] }) {
  const [form, setForm] = useState({ name: '', phone: '', email: '', propertyId: '', source: 'WALK_IN', roomType: '', messIncluded: false, notes: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onClose();
      setForm({ name: '', phone: '', email: '', propertyId: '', source: 'WALK_IN', roomType: '', messIncluded: false, notes: '' });
    } catch {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <div className="mx-auto max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden relative z-10">
          <div className="flex justify-between items-center p-5 border-b border-surface-border">
            <h2 className="text-lg font-bold text-text-primary">New Enquiry</h2>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div>
              ...
              <textarea className="input-field resize-none h-20" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Budget, move-in date, etc..." />
            </div>
